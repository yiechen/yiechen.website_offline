...
author: "Nick Ray, David Hoagey, Linh Lazarus, Ekarin Pongpipat, & Matt Kmiecik"
date: "20 March 2018"
output: 
  html_document:
    highlight: zenburn
...
```{r}
library(tidyverse); library(repmis); library(RCurl)
```
...
```{r}
library(tidyverse); library(RCurl); library(repmis)
```
...
```{r, echo=FALSE}
kable(bind_cols(data.frame(Seed = colnames(ProportionsData)),
                ProportionsData)) %>%
  kable_styling(bootstrap_options = c('striped', 'hover', 'responsive')) %>%
  scroll_box(height = "400px")
```
