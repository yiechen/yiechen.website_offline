...
```{r excel}
Author <- c('Tolchok & Krovvy', 'Hoagey, Ray & Miranda', 'Mulligan & Belmont')
Year <- c(1962, 2017, 2016)
Title <- c('Milkbars and Ultra-Violence', 'The Woes of Graduate Life', 'Ahead of their Time')
Type <- c('review', 'experimental', 'meta-analysis')
Category <- c('Category A', 'Category A', 'Category B')
SubCategory <- c('Category A-1', 'Category A-2', 'Category B-1')
Summary <- c('This article is about...', 'This article is about...', 'This article is about...')

excel <- data.frame(Author, Year, Title, Type, Category, SubCategory, Summary)
...
I then would import the paper into [EndNote](http://endnote.com/) and ensure the citation was correct. I then would read the paper and take notes in a Word document, with roughly 1 Word doc page per paper. 
...
1) Author(s) - date - title of paper
2) Keywords of the paper (tip: create your own if they are not in the paper already)
3) Intro - things I learned
4) Methods - important info to understand the results
5) Results/Discussion - main findings and bottom line
...
As mentioned in the "Have a System" section, I used [EndNote](http://endnote.com/) to manage my citations. Citation managers are software programs that organize your papers/citations for easy citing while writing. Some other common reference managers are [Mendeley](https://www.mendeley.com/) and [RefWorks](https://www.refworks.com/refworks2/default.aspx?r=authentication::init&groupcode=RWUTexDallas). 
...
Hopefully this was helpful and don't be shy to try me on things that worked/did not work for you!
